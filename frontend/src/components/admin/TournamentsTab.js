...
export const TournamentsTab = ({ data, onRefresh }) => {
  const [form, setForm] = useState({ name: '', sport_type: '', date: '', results: '', winners: '', images: [] });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`${API}/tournaments/${editing.id}`, { ...form, id: editing.id });
      } else {
        await axios.post(`${API}/tournaments`, form);
      }
      toast.success(editing ? 'Updated' : 'Added');
      setForm({ name: '', sport_type: '', date: '', results: '', winners: '', images: [] });
      setEditing(null);
      onRefresh();
    } catch (error) {
      toast.error('Failed');
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{editing ? 'Edit' : 'Add'} Tournament</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input value={form.sport_type} onChange={(e) => setForm({...form, sport_type: e.target.value})} required />
            </div>
          ...
          <div>
            ...
            <Textarea value={form.results} onChange={(e) => setForm({...form, results: e.target.value})} rows={2} required />
          </div>
          <div>
            <label className="block text-sm font-semibold mb-2">Winners</label>
            <Textarea value={form.winners} onChange={(e) => setForm({...form, winners: e.target.value})} rows={2} required />
          </div>
          <div>
          ...
          <div className="flex space-x-2">
            ...
            {editing && <Button type="button" variant="outline" onClick={() => { setEditing(null); setForm({ name: '', sport_type: '', date: '', results: '', winners: '', images: [] }); }}><X className="mr-2" size={18} />Cancel</Button>}
          </div>
        </form>
            ...
            {data.map((item) => (
              <div key={item.id} className="bg-gray-50 rounded-lg p-4 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600">{item.sport_type} - {item.date}</p>
                </div>
                <div className="flex space-x-2">
